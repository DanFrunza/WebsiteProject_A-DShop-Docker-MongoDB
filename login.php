...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $secretKey = "6LfpzRMpAAAAAG-ajMLxFCmjNyn5f3ZJMeser_r7"; // Înlocuiește cu cheia secretă reCAPTCHA
    ...
    if ($responseData->success) {
        ...
        if (empty($username)) {
            ...
            // Verifică existența utilizatorului în colecția "users"
            $query = new MongoDB\Driver\Query(['user' => $username]);
            $rows = $client->executeQuery("database1.users", $query);
            ...
            if ($user) {
                ...
                // Utilizatorul nu există, adăugăm un nou utilizator în colecție
                $hash = password_hash($password, PASSWORD_DEFAULT);
                $bulk = new MongoDB\Driver\BulkWrite();
                
                $registration_date = date("Y-m-d H:i:s");
                $bulk->insert(['user' => $username, 'password' => $hash, 'registration_date' => $registration_date]);

                $client->executeBulkWrite("database1.users", $bulk);
